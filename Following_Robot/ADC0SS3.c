...
void ADC0_Seq3_Ch9_Init(void){
  ...
  GPIO_PORTE_AFSEL_R |=  0x10;   // alt function on PE4
  GPIO_PORTE_DEN_R   &= ~0x10;   // disable digital on PE4
  GPIO_PORTE_AMSEL_R |=  0x10;   // enable analog on PE4

  // 3) Enable ADC0
  SYSCTL_RCGC0_R |= 0x00010000;  // activate ADC0
  while((SYSCTL_RCGC0_R & 0x00010000) == 0) {}

  // 4) Configure ADC0 for 125 ksps, SS3 highest priority, software trigger
  SYSCTL_RCGC0_R &= ~0x00000300; // 125K samples/sec
  ADC0_SSPRI_R    = 0x0123;      // SS3 highest priority
  ADC0_ACTSS_R   &= ~0x0008;     // disable SS3 during config
  ADC0_EMUX_R    &= ~0xF000;     // SS3 trigger = processor (software)
  ADC0_SSMUX3_R   = (ADC0_SSMUX3_R & 0xFFFFFFF0) + 9; // AIN9 (PE4)
  ADC0_SSCTL3_R   = 0x0006;      // IE0=1, END0=1 (no TS0, no D0)
